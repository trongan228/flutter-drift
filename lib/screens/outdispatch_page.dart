...
class OutdispatchPage extends StatefulWidget {
  const OutdispatchPage({super.key, required this.database});
  final AppDatabase database;
  @override
  State<OutdispatchPage> createState() => _HomePageState();
}

class _HomePageState extends State<OutdispatchPage> {
  int? selectedYear;
  ...
  late OutcomingDispatchDataSource _outDataSource;
  late Future<List<OutcomingDispatch>> _outcomingFuture;
  ...
  void _insertDocument() {
    OutcomingDispatchsCompanion outcoming = OutcomingDispatchsCompanion(
      number: Value(_numberController.text),
      docDate: Value(_dateController.text),
      name: Value(_nameController.text),
      signer: Value(_signerController.text),
      receiver: Value(_receiverController.text),
    );
    widget.database.into(widget.database.outcomingDispatchs).insert(outcoming);
    _updateTable();
  }
  ...
  void convertExceltoJson(String path) async {
    ...
    int maxId = await widget.database.getMaxIdOutcomingDispatch();
    List<OutcomingDispatch> documents = jsonData.map((data) {
      maxId += 1;
      return OutcomingDispatch(
        id: maxId,
        number: data['Số văn bản']?.toString() ?? '',
        docDate: data['Ngày văn bản']?.toString() ?? '',
        name: data['Tên văn bản']?.toString() ?? '',
        signer: data['Người ký']?.toString() ?? '',
        receiver: data['Nơi nhận']?.toString() ?? '',
        quantity: data['Số lượng']! ?? 0,
        warehouseId: data['Số thùng']?.toString() ?? '',
      );
    }).toList();
    await widget.database.insertAllOutDispatch(documents);
    _updateTable();
  }
  ...
  Future<List<OutcomingDispatch>> getData() async {
    ...
    //   return docs;
    // }
    List<OutcomingDispatch> docs = await widget.database
        .filterOut(this.selectedYear, _textSearchController.text);
    return docs;
  }

  Future<List<OutcomingDispatch>> getDocumentByDate(DateTime selected) async {
    // if (_textSearchController.text.isNotEmpty) {
    //   print(_textSearchController.text);
    //   List<OutcomingDispatch> docs =
    //       await widget.database.searchDocument(_textSearchController.text);
    ...
    // }
    List<OutcomingDispatch> docs =
        await widget.database.getAllOutcomingDispatch();
    return docs;
  }
    ...
    return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            FutureBuilder<List<OutcomingDispatch>>(
                future: _outcomingFuture,
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    _outDataSource =
                        OutcomingDispatchDataSource(docs: snapshot.data!);
                    return SizedBox(
                      height: 600,
                      child: Column(
